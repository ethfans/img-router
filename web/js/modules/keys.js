...
import { apiFetch, escapeHtml, providerMeta, detectApiKey } from './utils.js';
...
async function loadProviders() {
    try {
        ...
        providers.forEach(p => {
            ...
            div.innerHTML = `
                <i class="${meta.icon}" style="font-size: 18px; color: ${isActive ? 'var(--primary)' : meta.color};"></i>
...
async function switchProvider(name) {
    ...
    document.getElementById('currentProviderDesc').innerText = meta.desc || `管理 ${name} 模型的 API Key`;
...
async function loadKeys(provider) {
    ...
    try {
        ...
        keyPool.forEach(k => {
            ...
            const successRate = k.totalCalls > 0 ? Math.round((k.successCount / k.totalCalls) * 100) + '%' : '-';

            tr.innerHTML = `
                <td style="padding: 16px 24px;">
                    <div style="font-weight: 500; color: var(--text-primary);">${escapeHtml(k.name)}</div>
                    <div style="font-size: 12px; color: var(--text-secondary);">ID: ${k.id.slice(0, 8)}</div>
                </td>
                <td style="padding: 16px 24px;">
                    <code style="background: rgba(0,0,0,0.05); padding: 2px 6px; border-radius: 4px; font-size: 12px;">${escapeHtml(keyDisplay)}</code>
                </td>
                <td style="padding: 16px 24px;">
                    <label class="switch" style="transform: scale(0.8); transform-origin: left center;">
                        <input type="checkbox" ${k.enabled ? 'checked' : ''} onchange="globalThis.toggleKeyStatus('${k.id}', this.checked)">
                        <span class="slider"></span>
                    </label>
                    <span style="font-size: 12px; color: ${k.enabled ? 'var(--success)' : 'var(--text-secondary)'}; margin-left: 8px;">
                        ${k.enabled ? '已启用' : '已禁用'}
                    </span>
                </td>
                <td style="padding: 16px 24px;">
        ...
        console.error(e);
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding: 20px; color: var(--error);">加载失败: ${e.message}</td></tr>`;
    }
}
...
async function confirmAddKey() {
    ...
    if (invalidKeys.length > 0) {
        alert(`检测到 ${invalidKeys.length} 个 Key 不符合 ${currentProvider} 的格式要求：\n${invalidKeys.slice(0, 3).join('\n')}${invalidKeys.length > 3 ? '\n...' : ''}\n请检查是否选择了正确的渠道。`);
        return;
    }
