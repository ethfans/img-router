...
import { apiFetch, providerMeta, detectApiKey, escapeHtml } from './utils.js';
...
function renderDashboardChannels(config) {
    ...
    for (const p of providers) {
        const meta = providerMeta[p.name] || { icon: "ri-server-line", color: "#666", desc: "未知渠道" };
        const isEnabled = p.enabled !== false;
        
        let isConfigured = false;
        if (p.name === 'Doubao') isConfigured = config.doubaoConfigured || detectApiKey(config.globalAccessKey, 'Doubao');
        else if (p.name === 'Gitee') isConfigured = config.giteeConfigured || detectApiKey(config.globalAccessKey, 'Gitee');
        else if (p.name === 'ModelScope') isConfigured = config.modelscopeConfigured || detectApiKey(config.globalAccessKey, 'ModelScope');
        else if (p.name === 'HuggingFace') isConfigured = config.hfConfigured || detectApiKey(config.globalAccessKey, 'HuggingFace');
    ...
    if (lastLogEntry && 
        ...
        if (lastLogElement) {
            ...
            if (countBadge) {
                ...
                badge.style.cssText = 'margin-left: 8px; font-size: 10px; background: rgba(255,255,255,0.1); padding: 0 4px; border-radius: 4px; color: var(--text-secondary);';
